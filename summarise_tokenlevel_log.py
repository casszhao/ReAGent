...
def get_one_model(model_name):
    ...
    lime_suff, lime_comp, random_lime_suff, random_lime_comp = get_one_line_for_one_FA(model_name, "lime")
    ours_suff, ours_comp, ours_random_all_suff, ours_random_all_comp = get_one_line_for_one_FA(model_name, "ours")

    suff_df = pd.DataFrame([signed_suff, integrated_suff, shap_suff, rollout_suff, last_suff, all_suff, lime_suff, ours_suff], columns=['FAs', 'Soft Suff'])
    comp_df = pd.DataFrame([signed_comp, integrated_comp, shap_comp, rollout_comp, last_comp, all_comp, lime_comp, ours_comp], columns=['FAs', 'Soft Comp'])

    random_suff_df = pd.DataFrame([random_signed_suff, random_integrated_suff, random_shap_suff, random_rollout_suff, random_last_suff, random_all_suff, random_lime_suff, ours_random_all_suff], columns=['FAs', 'Soft Suff'])
    random_comp_df = pd.DataFrame([random_signed_comp, random_integrated_comp, random_shap_comp, random_rollout_comp, random_last_comp, random_all_comp, random_lime_comp, ours_random_all_comp], columns=['FAs', 'Soft Comp'])
    ...
    final_suff_df['Soft Comp']=final_comp_df['Soft Comp']
    final_suff_df.to_csv(f'evaluation_results/summary/{model_name}/{hyper}_minus.csv')
...
df.to_csv('evaluation_results/summary/token_level_all_lime.csv')


suff = df[["FAs","Soft-NS","Model"]]
comp = df[["FAs","Soft-NC","Model"]]
print(' ')
print(' ')
print(comp)
...
fig.legend(handles[:9], labels[:9], ncol=4, loc='center', bbox_to_anchor=(0.5, 0.21), fontsize=9) # 00 0.4 middle 0.8 top
...
plt.savefig(f"./evaluation_results/summary/token_lime_{hyper}.png",bbox_inches='tight')
print(f"./evaluation_results/summary/token_lime_{hyper}.png")
