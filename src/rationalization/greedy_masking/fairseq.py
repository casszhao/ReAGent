...
def baseline_rationalize_conditional_model(model,
    ...
    for target_token in range(num_target_tokens - 1):
        ...
        if method == 'gradient_norm':
            ...
            encoder_grad_norm = torch.sum(
                encoder_grad * encoder_embedding, -1)[0]
            decoder_grad_norm = torch.sum(
                decoder_grad * decoder_embedding, -1)[0]
            concatenated_tokens = torch.cat(
                [encoder_grad_norm, decoder_grad_norm])
            # Make sure we always add the most recent token first.
            concatenated_tokens[-1] = float("inf")
            ...
            decoder_self_attns = torch.stack(
                full_decoder_out[1]['self_attns'], 0)
            decoder_cross_attns = torch.stack(
                full_decoder_out[1]['cross_attns'], 0)
            ...
            # Average over layers
            decoder_self_attns = torch.mean(decoder_self_attns, 0)[
                0, target_token]
            decoder_cross_attns = torch.mean(decoder_cross_attns, 0)[
                0, target_token]
            concatenated_tokens = torch.cat(
                [decoder_cross_attns, decoder_self_attns])
            # Make sure we always add the most recent token first.
        ...
        for rationale_size in range(1, max_rationale_size):
            ...
            if len(source_rationale) == 0:
                ...
                encoder_positions = all_source_positions[0,
        ...
        all_source_rationales.append(
            list(unsorted_source_rationale.cpu().numpy()))
        all_target_rationales.append(
            list(unsorted_target_rationale.cpu().numpy()))
        ...
        for rationale_size in range(1, max_rationale_size):
            if rationale_size == 1:
                ...
                decoder_out = model.model.decoder(
                    ...
                    encoder_out=encoder_out,
                    position_ids=all_target_positions[:, target_token:(
                        target_token + 1)]
                )
                ...
                only_rationalize_source = len(
                    target_rationale) == target_token + 1
                only_rationalize_target = len(
                    source_rationale) == num_source_tokens
                ...
                if not only_rationalize_source:
                    ...
                    num_target_candidates = (
                        target_token + 1) - len(target_rationale)
                ...
                if not only_rationalize_source:
                    ...
                    decoder_positions = all_target_positions[0,
                                                             target_candidates]
                ...
                if not only_rationalize_target:
                    ...
                    num_source_candidates = num_source_tokens - \
                        len(source_rationale)
                    ...
                    decoder_positions_padded = all_target_positions[0,
                                                                    original_indices]
                ...
                if only_rationalize_target:
                    ...
                    decoder_input = torch.cat(
                        [decoder_input_padded, decoder_input], 0)
    ...
    for prev_token in range(start_step, num_tokens - 1):
        ...
        for rationale_size in range(1, min(max_steps + 1, prev_token + 2)):
            if rationale_size == 1:
                ...
                best_token_position = set(
                    candidates[best_token]) - set(rationale)
                best_token_position = best_token_position.pop()
