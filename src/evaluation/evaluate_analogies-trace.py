...
def main():
    ...
    parser.add_argument("--importance_results_dir", 
                        type=str,
                        default="rationalization_results/analogies/test", #ours/top5_replace0.2_max3000_batch5",
                        help="path for storing the importance scores extracted") # TODO
    parser.add_argument("--eva_output_dir", 
                        type=str,
                        default="evaluation_results/analogies/test", #ours/top5_replace0.2_max3000_batch5",
                        help="") # TODO
    ...
    parser.add_argument("--logfolder", 
                        type=str,
                        default=None, #gpt2_ours/top5_replace0.2_max3000_batch5',
    ...
    with open(os.path.join(output_dir, f'details_{args.rationale_size_ratio}.csv'), "w", newline="") as csv_details_f:
        ...
        for filename in filenames:
            ...
            importance_scores_all = torch.tensor([rationalization_result["trace"]["importance-scores"]], device=device)
            random_importance_scores = normalise_random(torch.rand([importance_scores_all.shape[0], importance_scores_all.shape[2]], device=device))
            ...
            for step in torch.arange(importance_scores_all.shape[1]):

                importance_scores = importance_scores_all[:, step]
                ...
                logging.info(f"{filename} {step.item()} - {norm_suff.item()}, {norm_comp.item()}, {random_norm_suff.item()}, {random_norm_comp.item()}")
                metric = [identifier, step.item(), norm_suff.item(), norm_comp.item(), random_norm_suff.item(), random_norm_comp.item()]
                metrics.append(metric)

                details_writer.writerow(metric)
                csv_details_f.flush()

    metrics_rm_id = [sublist[2:] for sublist in metrics]
